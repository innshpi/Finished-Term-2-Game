...
public class Term2Project
{
        ...
        while(shipShoot){
            ...
            if (cmd0.length() == 2 && cmd0.charAt(0) >= '0' && cmd0.charAt(0) <= '9' && cmd0.charAt(1) >= 'a' && cmd0.charAt(1) <= 'j'){
                ...
                playingBoardPlayer2[shootY][shootX] == shipChar[5] && player2Playing){
                    ...
                    System.out.println('\u000C');
                    if(player1Playing)playingBoardPlayer1();
                    if(player2Playing)playingBoardPlayer2();
    ...
    //player1Shoot operates player 2's turn to shoot
    ...
    public void sink2Ship(){
        //check if a ship has been sunk
        if (placingBoardPlayer1[shootY][shootX] == shipChar[0])s1P2Hit--;if (s1P2Hit == 0)shipP2Sink[0] = 1;
        if (placingBoardPlayer1[shootY][shootX] == shipChar[1])s2P2Hit--;if (s2P2Hit == 0)shipP2Sink[1] = 1;
        if (placingBoardPlayer1[shootY][shootX] == shipChar[2])s3P2Hit--;if (s3P2Hit == 0)shipP2Sink[2] = 1;
        if (placingBoardPlayer1[shootY][shootX] == shipChar[3])s4P2Hit--;if (s4P2Hit == 0)shipP2Sink[3] = 1;
        if (placingBoardPlayer1[shootY][shootX] == shipChar[4])s5P2Hit--;if (s5P2Hit == 0)shipP2Sink[4] = 1;
    }
    ...
    public void winGame(){
        ...
        System.out.println("Please type finish and restart the game to play again");
        while(i>=0){    
            String cmd0 = kb.nextLine();
            cmd0 = cmd0.toLowerCase();
            if(cmd0.equals("finish")){
                kb.close();
                System.exit(0);
            }
        }
